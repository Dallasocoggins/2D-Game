...
public class EnemyAI : MonoBehaviour
{
    ...
    //the max dist from ai to a waypoint for it to continue to the next waypoint
    ...
    {
        GameObject sResult = GameObject.FindGameObjectWithTag(SearchForTag);
        if (sResult == null)
        {
    ...
    {
        Debug.Log("We have got a path. Did it have an error? " + p.error);
        if (!p.error)
        {
            path = p;
            currentWaypoint = 0;
